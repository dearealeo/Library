...
upgrade_tools() {
    ...
    local tools
    tools=$(LC_ALL=C "$UV_BIN" tool list 2>/dev/null | awk 'NR>1 && $1 ~ /^[a-zA-Z0-9]/ {print $1}')
    [[ -z "$tools" ]] && { log "No tools installed, nothing to upgrade"; return 0; }
    
    log "Found installed tools, proceeding with upgrades"
    while IFS= read -r tool; do
        [[ -z "$tool" || "$tool" == "-" || ! "$tool" =~ ^[a-zA-Z0-9] ]] && continue
        log "Upgrading: $tool"
        "$UV_BIN" tool upgrade "$tool" --quiet 2>/dev/null || log "Warning: Failed to upgrade $tool, continuing"
    done < <(printf '%s\n' "$tools")
}
...
upgrade_packages() {
    ...
    while IFS= read -r package; do
        ...
        "$UV_BIN" pip install -U "$package" --quiet 2>/dev/null || log "Warning: Failed to upgrade package $package, continuing"
    done < <(printf '%s\n' "$outdated_packages")
}
